...
inline void mass_generate(int n, int mc, int MC, const std::string& of_pwd, const std::string& of_hash)
{
    bool genHash = (of_hash != "");
    ...
    if(passwd_file.is_open() && (!genHash || hash_file.is_open()))
	{		
		for(int i = 0; i < n; i++)
		{
